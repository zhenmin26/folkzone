...
describe("test article", () => {
  ...
  it("should fetch all articles for current logged in user", () => {
    // store.dispatch({type: "getpostsInUser", data: "Bret"})
    const allPosts = [
      {
        userId: 1,
        id: 1,
    ...
    const res = userReducer(
      { initState },
      { type: "getAllPostsInUser", data: allPosts }
    );
    expect(res.allPostsInUser.length).toEqual(100);
  });
  ...
  it("should add articles when adding a follower", () => {
    userReducer({initState, friendUserIds: [1,2,3]}, {type: "addFriend", data: [5]})
    expect(userReducer({initState, friendUserIds: [1,2,3]}, {type: "addFriend", data: [5]})).toEqual({
      ...initState,
      friendUserIds: [1,2,3,5]
    });
  });
